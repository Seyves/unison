...
const ChatPreview = ({id, name, avatar, lastMessage, unreadCount} : ChatPreviewComponent) => {
    ...
    const lastMesTime = getTimeFromISO(lastMessage?.createdAt)
    ...
    if (isFromMe) {
        ...
        indicator = <div className="flex justify-center"><StatusIcon status={messageStatus}/></div>
    } else {
    ...
    return (
        <Link to={"/messenger/" + id} className="flex h-16 p-2 cursor-pointer hover:bg-stone-700 transition ">
            <Person 
                name={name} 
                avatar={avatar} 
                placeholder={(isFromMe ? 'You: ' : '') + lastMessage.text}
            />
